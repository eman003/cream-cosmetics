<template>
    <app-layout>
        <div class="text-center uppercase text-2xl ">update ambassador</div>
                    ...
                    <div class="pt-6 mb-4 flex flex-col">
                        <div class="-mx-3 md:flex mb-6">
                            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                                ...
                                <input class="w-full bg-gray-50 text-black border border-gray-200 focus:border focus:border-green-400 rounded py-3 px-4 mb-3" id="name" v-model="form.name" :class="errors.name?'border-red-500':null" type="text" >
                            </div>
                            <div class="md:w-1/2 px-3">
                                <label class="uppercase tracking-wide text-black text-xs font-bold mb-2" for="status">
                                    status*
                                </label>
                                <select v-model="form.status_id" :class="errors.status_id?'border-red-500':null" class="w-full bg-gray-50 text-black border border-gray-200 focus:border focus:border-green-400 rounded py-3 px-4 mb-3" id="status">
...
export default {
    ...
    data(){
        return {
            image: {backgroundImage: 'url(\'/storage/'+this.ambassador.image+'\')'},
            alertOpen: true,
            form: this.$inertia.form({
                name: this.ambassador.name,
                status_id: this.ambassador.status_id,
                image: null
            }),
        }
    },
    methods: {
        submit(){
            this.form.put(this.route('ambassador.update', this.ambassador.id))
        }
    },
    /*setup (props) {
        const form = useForm({
        ...
        function submit() {
            Inertia.put('/ambassador/'+props.form.id, form)
        }
