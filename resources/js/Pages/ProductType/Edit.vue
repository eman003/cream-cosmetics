<template>
    <app-layout>
        <div class="text-center uppercase text-2xl ">Update product types</div>
                   ...
                   <div v-for="(field, index) in form">
                    ...
                    <div class="pt-3 mb-4 flex flex-col">
                        <div class="-mx-3 md:flex mb-6">
                            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                                ...
                                <input type="text" v-model="form.name" class="w-full bg-gray-50 text-black border border-gray-200 focus:border focus:border-green-400 rounded py-3 px-4 mb-3" id="name" :class="errors.name?'border border-red-500':null" >
                            </div>
                            <div class="md:w-1/2 px-3">
                                <label class="uppercase tracking-wide text-black text-xs font-bold mb-2" for="status">
                                    status*
                                </label>
                                <select  v-model="form.status_id" class="w-full bg-gray-50 text-black border border-gray-200 focus:border focus:border-green-400 rounded py-3 px-4 mb-3" :class="errors.status_id?'border border-red-500':null" id="status">
                    ...
                    <div class="-mx-3 md:flex mt-2">
                        <div class="md:w-full px-3">
                            <button class="md:w-full bg-green-400 active:outline-none focus:outline-none focus:border-green-400 uppercase hover:bg-green-500 shadow hover:shadow-md text-white font-bold py-2 px-4 rounded">
                                Update product type
                            </button>
                        </div>
                    </div>
...
export default {
    ...
    setup(props){
        ...
        function edit(){
            Inertia.put('/producttype/'+props.producttype.id, form)
        }
